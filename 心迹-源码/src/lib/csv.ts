  ...
  for (const r of state.statusRecords) {
    rows.push(
      [
        ...
        r.factorFree,
        joinArr(r.sensationParts),
        joinArr(r.sensationFeelings),
        joinArr(r.perceptions),
        r.sensationFree,
        r.recordedAt,
        r.isBackfill ? '1' : '0',
        '',
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (type === 'status') {
      ...
      statusRecords.push({
        ...
        sensationParts: splitArr(get(r, 'sensationParts')),
        sensationFeelings: splitArr(get(r, 'sensationFeelings')),
        perceptions: splitArr(get(r, 'perceptions')),
        sensationFree: get(r, 'sensationFree'),
        recordedAt,
